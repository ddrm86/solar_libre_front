...
test.beforeEach(async ({ page }) => {
  await page.goto('/inventory/panels');
})

test('has panels table', async ({ page }) => {
  await expect(page.getByText('Maker')).toBeVisible();
  await expect(page.getByText('Nominal Power')).toBeVisible();
})
...
test('clicking delete shows confirmation dialog', async ({ page }) => {
  await page.getByRole('button', { name: '' }).click();
  await expect(page.getByLabel('Confirm')).toContainText('Are you sure you want to delete this panel?️');
